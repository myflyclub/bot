...
if __name__ == "__main__":
    async def test_bot_main():
        """Test bot main function that simulates crashes"""
        print("🤖 Test bot starting...")
        
        # Simulate some work
        for i in range(5):
            print(f"Bot working... {i+1}/5")
            await asyncio.sleep(1)
        
        # Simulate a crash on 3rd run
        import random
        if random.random() < 0.7:  # 70% chance of crash
            raise RuntimeError("Simulated bot crash for testing")
        
        print("✅ Test bot completed successfully")
    
    async def test_supervisor():
        """Test the bot supervisor"""
        print("🧪 Testing Bot Supervisor")
        print("=" * 40)
        
        # Create supervised bot
        supervisor = create_supervised_bot(
            bot_main_func=test_bot_main,
            max_restart_attempts=3,
            restart_delay_base=2
        )
        
        print("✅ Supervised bot created")
        
        # Start the supervisor
        try:
            await supervisor.start()
        except KeyboardInterrupt:
            print("\n⏹️ Supervisor stopped by user")
        
        # Get final stats
        stats = supervisor.get_supervisor_stats()
        print("\n📊 Final supervisor stats:")
        for key, value in stats.items():
            if key == 'crash_handler_stats':
                print(f"   {key}: {len(value.get('crash_history', []))} crashes")
            else:
                print(f"   {key}: {value}")
        
        print("\n🎉 Supervisor tests completed!")
